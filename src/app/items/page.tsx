...
export default function ItemsPage() {
  const { items, addItem, updateItem, deleteItem, getItemById, getLinkedItems } = useItemStore()
  ...
  const linkedItems = useMemo(
    () => (selectedItem ? getLinkedItems(selectedItem.id) : []),
    [selectedItem, getLinkedItems, items]
  )
      ...
      {selectedItem && (
        <div className="w-full lg:w-96 flex-shrink-0 h-full">
          <ItemDetail
            ...
            onLinkedItemClick={(item) => setSelectedItem(item)}
          />
        </div>
      )}
